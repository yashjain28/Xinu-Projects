...
struct  topicent topictab[NTOPIC]; /* topic table */
struct  brokernode *headbroker=NULL;
struct brokernode *head1=NULL;
...
static	void	sysinit()
{
	...
	for(i=0;i < NTOPIC; i++) {
		topicptr=&topictab[i];
		for(j=0;j<NSUBS;j++){
			topicptr->substopic[j].procID=-1;
			topicptr->substopic[j].groupID=0;
			topicptr->substopic[j].handle=NULL;
		}
	}
