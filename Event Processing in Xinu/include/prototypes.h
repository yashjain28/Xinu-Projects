...
/* IN FILE PUBLISH.C */
extern syscall  subscribe(topic16  topic,  void  (*handler)(topic16,  uint32));	
extern syscall  unsubscribe(topic16  topic);	  
extern syscall  publish(topic16  topic,  uint32  data);	
  

/* IN FILE BROKEROP.C */
extern syscall	addnodeblist(topic16 topicid, uint32 mesg);
extern syscall	delnodeblist();
extern syscall printblist();

extern syscall  handlerprint(topic16  topic, uint32 mesg);
extern syscall  unsubscribe(topic16 topic);
extern process  Broker(void);
extern syscall 	unsubkill(pid32 prociD);
//	extern syscall	unsubs(pid32 proci);
